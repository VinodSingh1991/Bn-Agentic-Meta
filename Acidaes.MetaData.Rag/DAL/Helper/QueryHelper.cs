namespace Acidaes.MetaData.Rag.DAL.Helper
{
    ...
    {
        public const string RoleRagDocumentQuery = "select * from RoleView";
        ...
        public const string GetCustomFields = "select Label, FieldId, TableName, KeyId, Type, Description, FieldName, DisplayType from CustomFieldSchema where OwnerId = 101 and KeyId in(1,6,7,8,9)";
        public const string GetSystemFields = "select Label, LayoutFieldId, FieldId, TableName, KeyId, Type, Description, FieldName, DisplayType from ObjectSchema where OwnerId = 0 and KeyId in(1,6,7,8,9)";
